...
from coverletterer.services import email as email_service
from coverletterer.services import magic_link
...
def test_consume_expired_token_fails(monkeypatch):
    store = {}
    monkeypatch.setattr(magic_link, "create_token", _fake_create(store, ttl_minutes=-1))
    monkeypatch.setattr(magic_link, "consume_token", _fake_consume(store))
...
def _fake_create(store, ttl_minutes=15):
    import secrets
