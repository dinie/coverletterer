"""State that requests + finalizes a passwordless email (magic-link) sign-in.

The emailed link points at the backend's /auth/magic/verify route (see
auth_routes.py), which redirects to /auth/complete/[token]; this reads the
token (a LocalAuthSession id minted server-side) and writes it into the
LocalStorage auth_token so reflex_local_auth treats the user as authenticated.
"""
