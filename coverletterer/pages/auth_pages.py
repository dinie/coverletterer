"""Custom login page (username/password + optional magic link) and the
...
def login_page() -> rx.Component:
    """Login with the reflex-local-auth form plus optional magic-link sign-in."""
    return rx.center(
        rx.cond(
...
def auth_complete_page() -> rx.Component:
    """Shown briefly while we store the session token and redirect home."""
    return rx.center(
        rx.vstack(
            rx.spinner(size="3"),
