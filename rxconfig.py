...
# SMTP_*, DATABASE_URL, etc. available to the app. override=False so real
# shell env vars still win.
load_dotenv(override=False)


# Persistence: SQLite for local dev, Postgres-ready for production.
# Set DATABASE_URL (e.g. postgres://user:pass@host:5432/db) to override. Managed
# Postgres providers often hand out `postgres://…`; SQLAlchemy needs an explicit
# driver, so normalize to the psycopg (v3) dialect.
def _db_url() -> str:
